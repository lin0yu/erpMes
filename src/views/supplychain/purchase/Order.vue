  ...
  <div>
    <div style="overflow: hidden; margin-bottom: 16px;">
      <el-button type="primary" style="float: left; margin-bottom: 16px;"
        @click="showAddDialog = true">新增采购订单</el-button>
      <el-input v-model="filterText" placeholder="请输入订单编号或供应商" style="width: 240px; float: right; margin-bottom: 16px;"
        clearable />
    </div>
    ...
    <el-dialog v-model="showAddDialog" title="新增采购订单" width="400px">
      <el-form :model="addForm">
        ...
        <el-form-item label="金额">
          <el-input v-model="addForm.amount" type="number" />
        </el-form-item>
        <el-form-item label="状态">
          <el-input v-model="addForm.status" />
        </el-form-item>
        <el-form-item label="下单日期">
          <el-date-picker v-model="addForm.date" type="date" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="showEditDialog" title="编辑采购订单" width="400px">
      <el-form :model="currentRow" label-width="80px">
        ...
        <el-form-item label="下单日期"><el-input v-model="currentRow.date" /></el-form-item>
      </el-form>
      <template #footer>
...
const tableData = ref([
  { orderNo: 'PO20240101', supplier: '供应商A', amount: 10000, status: '已审核', date: '2024-01-15' },
  { orderNo: 'PO20240102', supplier: '供应商B', amount: 5000, status: '待审核', date: '2024-01-20' }
]);
const filterText = ref('');
const filteredData = computed(() => {
  if (!filterText.value) return tableData.value;
  return tableData.value.filter(row => row.orderNo.includes(filterText.value) || row.supplier.includes(filterText.value));
});
const showAddDialog = ref(false);
const showEditDialog = ref(false);
const addForm = ref({ orderNo: '', supplier: '', amount: '', status: '', date: '' });
const currentRow = ref({});
function handleAdd() {
  if (!addForm.value.orderNo || !addForm.value.supplier) return;
  tableData.value.push({ ...addForm.value });
  showAddDialog.value = false;
  addForm.value = { orderNo: '', supplier: '', amount: '', status: '', date: '' };
}
